# -----------------------------------------------------------------------
# Cancer Proteomics Analysis Pipeline (PDC000362)
#
...
# Read proteomic data
proteomic_data <- read_tsv("Data/CPTAC3_PTRC_FHCRC_HGSOC_Frozen_Validation_Proteome.tmt11.tsv")

# Read clinical data
clinical_data <- read_csv("Data/PDC_study_biospecimen_05202025_143559.csv")
...
colnames(log_intensity_data) <- colnames(log_intensity_data) %>% 
  str_remove_all("^Log\\s+") %>% 
  str_replace_all("[/.].*", "")

# Filter clinical samples
clinical_samples <- clinical_data %>% 
  filter(`Sample Type` %in% c("Primary Tumor", "Metastatic"),
         `Aliquot Status` == "Qualified")

# Create binary outcome vector (0: Primary, 1: Metastatic)
...
# Write to CSV file
write_csv(result_summary, "Outputs/Results/method_comparison_PDC000362.csv")
...
write_csv(feature_frequency, "Outputs/Results/STCS_feature_PDC000362.csv")
