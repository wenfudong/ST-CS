# -----------------------------------------------------------------------
# Cancer Proteomics Analysis Pipeline (PDC000356)
#
...
# Read proteomic data (log-transformed intensity values)
proteomic_data <- read_tsv("Data/iCC_NCC_Proteome.tmt10.tsv")

# Read clinical metadata (sample information)
clinical_data <- read_csv("Data/PDC_study_biospecimen_03212025_144732.csv")

# ----------------------------
# Data Preprocessing
# ----------------------------

# Process proteomics data matrix
log_intensity_data <- proteomic_data %>%
  column_to_rownames(var = "Gene") %>%          # Set gene symbols as row names
  select(starts_with("Log ")) %>%               # Select log-transformed features
  filter(!rownames(.) %in% c("Mean", "Median", "StdDev"))  # Remove summary rows

# Clean column names to keep sample IDs only
colnames(log_intensity_data) <- str_extract(
  colnames(log_intensity_data), 
  "(?<=Log\\s)[^:]+"
  )

# Filter clinical samples (Normal vs Tumor)
clinical_samples <- clinical_data %>% 
  filter(`Sample Type` %in% c("Normal Adjacent Tissue", "Human Tumor Original Cells"),
         `Aliquot Status` == "Qualified")

# Create binary outcome vector (0: Normal, 1: Tumor)
y <- ifelse(clinical_samples$`Sample Type` == "Normal Adjacent Tissue", 0, 1)

# Align proteomic data with clinical samples
sample_ids <- clinical_samples$`Sample Submitter ID`
...
# Write to CSV file
write_csv(result_summary, "Outputs/Results/method_comparison_PDC000356.csv")
...
write_csv(feature_frequency, "Outputs/Results/STCS_feature_PDC000356.csv")
